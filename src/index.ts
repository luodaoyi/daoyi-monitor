...
Type=oneshot
EnvironmentFile=$CONFIG_FILE
ExecStart=/bin/sh -c 'if command -v curl >/dev/null 2>&1; then curl -fsSL "\$DAOYI_AGENT_INSTALLER_URL"; else wget -qO- "\$DAOYI_AGENT_INSTALLER_URL"; fi | sh -s -- --endpoint "\$DAOYI_AGENT_ENDPOINT" --token "\$DAOYI_AGENT_TOKEN" --profile "\$DAOYI_AGENT_PROFILE" --interval "\$DAOYI_AGENT_INTERVAL_SEC" --manifest-url "\$DAOYI_AGENT_UPDATE_MANIFEST_URL" --installer-url "\$DAOYI_AGENT_INSTALLER_URL" --install-dir "$INSTALL_DIR" --config-file "$CONFIG_FILE"'
EOF
